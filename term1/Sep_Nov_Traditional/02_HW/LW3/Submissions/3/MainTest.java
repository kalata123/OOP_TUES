...
public class MainTest {
    ...
    private static void testRandomInitialization(TestResults results) {
        ...
        try {
            ...
            // Check border walls
            Field boardField = gameClass.getDeclaredField("board");
            boardField.setAccessible(true);
            char[][] board = (char[][]) boardField.get(gameInstance);
    ...
    private static void testMovementSystem(TestResults results) {
        ...
        try {
            ...
            String[] testCommands = {"w", "a", "s", "d", "up", "down", "left", "right"};
            ...
            for (String cmd : testCommands) {
                try {
                    ...
                    // Some moves might fail due to walls
                }
            }
    ...
    private static void testPlacementRules(TestResults results) {
        ...
        try {
            ...
            try {
                Method checkWallCapacity = gameClass.getDeclaredMethod("checkWallCapacity", int.class, int.class);
    ...
    private static void testBonusFeatures(TestResults results) {
        ...
        try {
            ...
            try {
                ...
                Field historyField = gameClass.getDeclaredField("history");
                historyField.setAccessible(true);
                
                Object history = historyField.get(gameInstance);
                if (history != null) {
            ...
            try {
                Method restart = gameClass.getDeclaredMethod("restart");
                Field initialBoardField = gameClass.getDeclaredField("initialBoard");
                initialBoardField.setAccessible(true);
                
                Object initialBoard = initialBoardField.get(gameInstance);
                if (initialBoard != null) {
                    System.out.println("   ✓ Restart functionality implemented (+2)");
                    bonusPoints += 2;
                }
            } catch (Exception e) {
                System.out.println("   ✗ Restart functionality test failed: " + e.getMessage());
            }
            
            // Test Level Loading from File (+3 points)
            try {
                Method loadLevelFromFile = gameClass.getDeclaredMethod("loadLevelFromFile", String.class);
                System.out.println("   ✓ Level loading from file implemented (+3)");
                bonusPoints += 3;
            } catch (NoSuchMethodException e) {
                System.out.println("   ✗ Level loading from file not implemented");
            }
            
            // Test Exception Handling (+3 points)
            try {
                Class<?> exceptionClass = Class.forName("InvalidLevelException");
            ...
            try {
                ...
                if (hasGameState) {
                    System.out.println("   ✓ GameState class for undo implemented (+2)");
                    bonusPoints += 2;
                }
            } catch (Exception e) {
