...
public class MainTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("Testing student implementation\n");
    ...
    private static void testInputHandling(TestResults results) {
        ...
        try {
            // Test input validation by running the main method with invalid inputs
            ProcessBuilder pb = new ProcessBuilder("java", "SokobanGame");
            Process process = pb.start();
            
            // Send invalid inputs followed by valid ones
            try (PrintWriter writer = new PrintWriter(process.getOutputStream())) {
                writer.println("4");  // Invalid width
                writer.println("5");  // Valid width
                writer.println("4");  // Invalid height  
                writer.println("5");  // Valid height
                writer.println("10"); // Too many boxes for 5x5
                writer.println("1");  // Valid boxes
        ...
        try {
            ...
            for (String cmd : testCommands) {
                try {
                    ...
                    // Some moves might fail due to walls/boxes
                }
            }
            
            if (successfulTests >= 2) {
                System.out.println("   ✓ Movement commands work");
                results.score += 2;
            }
    ...
    private static void testPlacementRules(TestResults results) {
        ...
        try {
            ...
            // Test R3: Check wall capacity implementation
            try {
                Field targetsOnWallField = gameClass.getDeclaredField("targetsOnWall");
                Field boxesOnWallField = gameClass.getDeclaredField("boxesOnWall");
                targetsOnWallField.setAccessible(true);
                boxesOnWallField.setAccessible(true);
                
                int[] targetsOnWall = (int[]) targetsOnWallField.get(gameInstance);
                int[] boxesOnWall = (int[]) boxesOnWallField.get(gameInstance);
                
                // Check if wall capacity arrays exist and have correct length
                if (targetsOnWall != null && boxesOnWall != null && 
                    targetsOnWall.length == 4 && boxesOnWall.length == 4) {
                    System.out.println("   ✓ R3: Wall capacity constraint implemented (2 points)");
                    results.score += 2;
                } else {
                    System.out.println("   ✗ R3: Wall capacity arrays not properly initialized");
                }
            } catch (NoSuchFieldException e) {
    ...
    private static void testBonusFeatures(TestResults results) {
        ...
        try {
            ...
            // Test Restart Functionality (+2 points)
            try {
                Method restart = gameClass.getDeclaredMethod("restart", int.class, int.class, int.class);
                System.out.println("   ✓ Restart functionality implemented (+2)");
                bonusPoints += 2;
            } catch (NoSuchMethodException e) {
                System.out.println("   ✗ Restart functionality not implemented");
            }
            
            // Test Wall Tracking System (+3 points)
            try {
                Field targetsOnWallField = gameClass.getDeclaredField("targetsOnWall");
                Field boxesOnWallField = gameClass.getDeclaredField("boxesOnWall");
                Method updateBoxWallCounts = gameClass.getDeclaredMethod("updateBoxWallCountsForMove", 
                    int.class, int.class, int.class, int.class);
                
                if (targetsOnWallField != null && boxesOnWallField != null && updateBoxWallCounts != null) {
                    System.out.println("   ✓ Advanced wall tracking system implemented (+3)");
                    bonusPoints += 3;
                }
            } catch (Exception e) {
                System.out.println("   ✗ Advanced wall tracking system not implemented");
            }
            
            // Check for main method with input handling (+2 points)
            try {
                Method mainMethod = gameClass.getDeclaredMethod("main", String[].class);
                System.out.println("   ✓ Complete game with main method implemented (+2)");
                bonusPoints += 2;
            } catch (Exception e) {
                System.out.println("   ✗ Main method not properly implemented");
            }
